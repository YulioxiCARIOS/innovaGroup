...
function mostrarCarrito(){
  ...
  if (carrito.length === 0) {
    ...
    carrito.forEach((producto) => {
      ...
      const caracteristicasElement = document.createElement("p");
      caracteristicasElement.textContent = producto.Caracteristicas;
...
function decrementarCantidad(prodId){
...
function incrementarCantidad(prodId){
  const producto = carrito.find((prod) => prod.id === prodId);
  if (producto) {
    producto.cantidad++;
    mostrarCarrito(); // Actualizar la interfaz
...
function aplicarDescuento() {
  ...
  if (cupon === "descuento10") { // Reemplaza "DESCUENTO15" con tu código de cupón válido
    const totalPagar = calcularTotalPagar();
    const descuento = totalPagar * 0.10; // Calcular el descuento como el 15% del total a pagar
